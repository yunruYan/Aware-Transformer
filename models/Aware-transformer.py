...
from models.backbone.resnet_backbone import resnet101
from models.backbone.vgg_backbone  import vgg as create_model
from models.encoder_decoder.AwareT_encoder import Encoder
from models.encoder_decoder.AwareT_decoder import Decoder
...
class PureT(BasicModel):
    ...
    def forward(self, **kwargs):
        ...
        # max_len=kwargs[cfg.PARAM.MAX_LEN]
        ...
        '''并行'''
        # gx, c_att, s_att = self.encoder(att_feats, att_mask)
        '''Channel-Spatial'''
        gx,c_att = self.encoder(att_feats, att_mask)
        decoder_out = self.decoder(gx, seq, c_att, seq_mask, att_mask)
        return F.log_softmax(decoder_out, dim=-1)

    def get_logprobs_state(self, **kwargs):
        wt = kwargs[cfg.PARAM.WT]
        state = kwargs[cfg.PARAM.STATE]
        c_att = kwargs[cfg.PARAM.ATT_FEATS]
        s_att = kwargs[cfg.PARAM.P_ATT_FEATS]
        ...
        seq_mask = subsequent_mask(ys.size(1)).to(c_att.device).type(torch.cuda.FloatTensor)[:, -1, :].unsqueeze(1)

        # [B, 1, Vocab_Size] --> [B, Vocab_Size]
        '''并行'''
        # decoder_out = self.decoder(gx, ys[:, -1].unsqueeze(-1), c_att, s_att, seq_mask, att_mask).squeeze(1)
        '''Channel-Spatial'''
        decoder_out = self.decoder(gx, ys[:, -1].unsqueeze(-1), s_att, seq_mask, att_mask).squeeze(1)
    ...
    def decode_beam(self, **kwargs):
        ...
        # gx, c_att, s_att = self.encoder(att_feats, att_mask)
        '''Channel-Spatial'''
        gx,s_att = self.encoder(att_feats, att_mask)
        # p_att_feats = self.decoder.precompute(encoder_out)

        state = None
        wt = Variable(torch.zeros(batch_size, dtype=torch.long).cuda())
        # kwargs[cfg.PARAM.ATT_FEATS] = c_att
        kwargs[cfg.PARAM.P_ATT_FEATS] = s_att
        ...
        for t in range(cfg.MODEL.SEQ_LEN):
            ...
            if t == 0:
                # expand input
                # c_att = utils.expand_tensor(c_att, beam_size)
                s_att = utils.expand_tensor(s_att, beam_size)
    ...
    # def attentionout(self,att_feats):
    #     # beam_size = kwargs['BEAM_SIZE']
    #     # greedy_decode = kwargs['GREEDY_DECODE']
    #     att_feats = torch.unsqueeze(att_feats,0).cuda()
    #     att_mask = torch.ones(att_feats.size()[0], 12*12).cuda()
    #     # att_mask = torch.ones( 12 * 12).cuda()
    #     batch_size = att_feats.size(0)
    #     att_feats = self.backbone(att_feats)
    #     att_feats = self.att_embed(att_feats)
    #     gx, c_att, s_att = self.encoder(att_feats, att_mask)
